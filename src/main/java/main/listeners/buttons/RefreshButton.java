...
public class RefreshButton extends ListenerAdapter {
    ...
    public void onButtonInteraction(@NotNull ButtonInteractionEvent event) {
        if (!event.getComponentId().equals("refresh")) return;
        ...
        if (Objects.equals(message.getEmbeds().get(0).getTitle(), "Open tickets")) {
            try {
                ...
                for (Ticket ticket : tickets) {
                    embed.addField(ticket.getTicketId().toString(), ticket.getTopic(), false);
                }
                embed.setTitle("Open tickets")
                        .setColor(0xff55ff)
                        .setTimestamp(OffsetDateTime.now());

                event.deferEdit().setEmbeds(embed.build()).setActionRow(Button.secondary("refresh", Emoji.fromUnicode("U+1F504"))).queue();
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else {
            ...
            try {
                ...
                if (ticket.isSolved()) {
                    event.deferEdit().setEmbeds(
                                    embed.setTitle(ticketId + " • Closed")
                                            .setColor(0xff55ff)
                    ...
                    event.deferEdit().setEmbeds(
                                    embed.setTitle(ticketId.toString())
                                            .setColor(0xff55ff)
                                            .setDescription(stringMention)
                                            .addField("Topic", ticket.getTopic(), false)
                                            .addField("Message", ticket.getMessage(), false)
